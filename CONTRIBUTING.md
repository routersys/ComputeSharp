...
| Generated HLSL, floating point constants | Formats a `const float` or `const double` with `IFormattable.ToString`, so a whole value loses its decimal point and becomes an integer literal, while the same value written as a literal keeps one; dividing by such a constant performs integer division | Appends the decimal point, and the `L` suffix for `double`, exactly as the literal path does | `9e20da42` |
