...
| `Hlsl.All`, `Hlsl.Any` and `Hlsl.Mul` | `All` and `Any` reduce Bool, Float and Int but not Double or UInt, and `Mul` declares Float and Int only, though HLSL accepts those kinds | `All` and `Any` reduce Double and UInt as well, and `Mul` declares UInt. Mul is left without Double on purpose: its three vector by vector rows fail DXIL validation the way `Dot` has no double overload, and a single `Double2x2` product loses the device at run time, so the kind is refused rather than half declared | `40fb5a27` |
...
| Generated HLSL, an integer matrix given to an intrinsic with an out parameter | Writes the call out and hands it to DXC, which terminates with an access violation and takes the compiler process with it, so the build fails with a native fatal error naming no source line | Refuses the call while rewriting, which skips the compilation step for that shader and reports `CMPW0123` at the author's own line. The condition is the shape of the signature rather than a name, since `modf`, `frexp` and `sincos` were each measured to terminate the same way. Both rewriters carry the refusal, an initializer reaching the compiler with the same call whenever the out argument is an identifier, so the judgment sits on the type they share. Direct2D is left alone, being compiled through FXC, which does not have the defect | `3ae0fa13`, `35b7edc7` |
...
| `DiagnosticInfo`, locations outside a syntax tree | Keeps only the syntax tree and the span of the location it is handed, so a location that names a file but belongs to no tree is discarded and the diagnostic reaches the author with no position at all. Nothing handed it such a location until the shader compilation moved out of the transform node, where a location can no longer be tree bound, and the four compile diagnostics of each descriptor generator lost their file and line | Captures such a location by value and rebuilds it when the diagnostic is created, so the file and line survive. A location inside a tree takes the same path as before, and a location that names no file, a metadata one, still yields none. The location it rebuilds belongs to no syntax tree, which is all a location can be at that point in the pipeline: binding one to a tree again would mean holding a tree in an incremental model, which compares unequal across unrelated edits and keeps a stale compilation alive. A suppression written in source for a single line is applied through the tree, measured by reporting two otherwise identical diagnostics inside the same suppressed span and finding only the tree bound one silenced, so the two warnings among the eight are silenced by a severity setting rather than in source. Retire this row once upstream carries the same repair | `10622a78` |
