...
| `ReflectionServices` DXC library resolver | Registers the resolver on the assembly that declares `ReflectionServices`, which declares no P/Invoke of its own, so it never runs. `dxcompiler` is found by the default probing either way, but `dxil.dll` is imported by no managed code at all: `dxcompiler.dll` loads it itself through `LoadLibrary`, which does not consult the app directory. Launching through the shared host then binds whatever copy is on `PATH`, or none at all, and the copy shipped beside the app goes unused | Registers it on the assembly that declares the `dxcompiler` import, so the pre-load happens and every launch mode binds the copy shipped beside the app. An assembly that already carries a resolver is treated as a lost race rather than a failed type initializer, that failure being cached and otherwise poisoning every later call | `5a28402b` |
