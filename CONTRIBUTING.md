...
| Generated HLSL, a static field initializer reaching itself | Rewrites the initializer of an external static field and adds the entry to the collection of static field definitions after the rewriting finishes, where the two other collections a rewriting can return to claim their entry before it. An initializer that reaches the field it initializes therefore finds no entry, rewrites the field a second time and adds it, and the outer rewriting adds the same key again when it returns. The generator faults with an argument exception, which discards the descriptors for every shader in the compilation unit and leaves the author with errors that name none of this. The two routes that close the cycle are a method and a constructor the initializer imports | Claims the entry before rewriting, the way the other two collections do, and treats a return to a claimed entry as a cycle, reporting `CMPW0124` on the compute path and `CMPWD2D0096` on the Direct2D one. Claiming alone was measured to be insufficient: the fault stops, no diagnostic is reported, and the shader compiler accepts generated HLSL that reads a global static through a function reading it back, so the fault would become a silently different value, which is the worse of the two in the order this tree keeps. The mark is an empty type declaration, which widens the type alias for a static field; the two places that read it both read an array built after every rewriting finishes, and building both generators over the widened alias reports no warning. One signature wrote the collection as a raw tuple rather than as the alias, which the widening turns into a nullability error, so it is aligned with the other three. The report is located on the read that closes the cycle and not on the field declaration: a cycle closed from two imported declarations is two places the author has to change, and one report on the declaration names neither of them. An initializer reading a static field directly, and a static field of the shader itself, reach neither the report nor the fault: both call sites exclude fields of the shader type, and such a field is collected into an array rather than into a dictionary. | `da160eb1`, `7ac9a03d`, `a1a12303`, `fabf4202` |
| Generated HLSL, a static field of the shader reaching itself | Collects the static fields of the shader by walking its members and keeps no record of the one being rewritten, so an initializer reading the field it writes has nothing to tell it apart from any other read. The generated HLSL then holds a global read inside its own initialization: where the read is written in the initializer, the shader compiler calls it uninitialized and the generator forwards that as a compilation error naming a line of generated source; where it goes through a declaration the initializer imports, the compiler sees nothing and the author is left with a value C# never computes, HLSL leaving the read uninitialized where C# defines it as the default of the type. The claim that lets an external field be told apart is made in the rewriter for bodies, which a field of the shader never reaches | Claims the entry in the processor that rewrites an initializer, which both entries reach, so a field of the shader is told apart the way an external one already is. The reads are answered in four places, one per shape of the read and per rewriter, and the test they share sits on the type both rewriters derive from; each of the four is measured by a mutation that fails only the row reaching it. The aggregation in each generator skips the fields of the shader, the claim having put them in the collection the external ones are read from while the walk above added them already, and the two aggregations are measured separately so that neither stands for the other. What is not refused is an initializer reaching the field through a method of the shader: such a method is not imported, its body being written by a pass that runs before any initializer is rewritten, so no claim exists yet where the cycle closes, and reordering the passes only moves the claim to where it is already filled in. Refusing that shape needs a walk of the call graph, which is outside what the rewriters hold | `698f73a8`, `b365ef70` |
