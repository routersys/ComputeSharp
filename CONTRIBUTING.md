...
| Generated HLSL, floating point constants | Formats a `const float` or `const double` with `IFormattable.ToString`. A whole value loses its decimal point and becomes an integer literal, so dividing by such a constant performs integer division, while the same value written as a literal keeps one. A non-finite value becomes `NaN` or `Infinity`, which HLSL does not declare | Appends the decimal point, and the `L` suffix for `double`, exactly as the literal path does, and writes a non-finite value as the reinterpretation of its bit pattern | `9e20da42`, `9b88c149` |
| Generated HLSL, reserved identifiers | Reserves the resource type names of Shader Model 5, but not `AppendStructuredBuffer`, `TextureBuffer`, `RayQuery`, `SubpassInput`, `SubpassInputMS`, the `FeedbackTexture2D` and `RasterizerOrdered` families, the geometry stream types, or the minimum precision scalars, so a captured field named after one of them reaches the compiler unrenamed and fails | Reserves the twenty names measured to collide, alongside the ones already listed | `46e199b3` |
