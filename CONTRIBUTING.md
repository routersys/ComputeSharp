...
| `D2D1ResourceTextureManagerImpl`, resource texture staging buffer initialization | When one allocation in a multi-step sequence (resource id, extents, extend modes, then the data and strides pair) succeeds and a later one in the same call fails, the successful allocation is neither freed nor cleared: a local variable that was never stored in a field is simply discarded, and a field that was already stored is left holding it. `Initialize`'s retry guard checks only `d2D1ResourceTexture` and `data`, so a retry either overwrites and leaks the earlier resource id, extents or extend modes buffer, or reaches `Update` with `data` set and `strides` still null, which dereferences the null `strides` pointer for a multi-dimensional texture | Frees any leftover resource id, extents or extend modes buffer before allocating a new one, frees a local extents buffer immediately if the following extend modes allocation fails, and rolls the data buffer back if the strides allocation after it fails, so a retry always starts from a consistent, fully-null state | `95055663` |

The ledger is kept current by derivation, not by memory: `build/audit-upstream-divergence.ps1` lists every commit after the marker below that modifies a file inherited from upstream and is not cited by a row. Run it before a release. When the queue is not empty, either add a row or decide that the commit is not a divergence, and then move the marker. The ledger is audited through commit `95055663`.
