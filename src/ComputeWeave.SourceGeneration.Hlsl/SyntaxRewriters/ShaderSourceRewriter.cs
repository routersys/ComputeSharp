    ...
    public override SyntaxNode VisitMemberAccessExpression(MemberAccessExpressionSyntax node)
    {
            ...
            SemanticModel.For(node).GetOperation(node, CancellationToken) is IMemberReferenceOperation operation)
        {
                ...
                HlslKnownProperties.TryGetAccessorRankAndAxis(propertyOperation.Property.GetFullyQualifiedMetadataName(), out int rank, out int axis))
            {
                ...
                // A constant buffer is written out as the value it holds, so it has no dimensions to query. The
                // report is before the cache below, the accessor being shared with the structured buffer types.
                // The Direct2D path declares no such type, so this is compiled only where one exists
                if (resourceType.Name == "ConstantBuffer")
                {
                    Diagnostics.Add(InvalidConstantBufferSizeAccess, node, propertyOperation.Property);
