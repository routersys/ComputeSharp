...
internal static class HlslOperatorsSyntaxProcessor
{
        ...
        [NotNullWhen(true)] out ExpressionSyntax? rewrittenNode)
    {
        ...
        // Also pre-filter just operators that are defined in ComputeWeave primitives
        if (operatorMethod is not { ContainingType.ContainingNamespace.Name: "ComputeWeave" })
        {
            rewrittenNode = null;
