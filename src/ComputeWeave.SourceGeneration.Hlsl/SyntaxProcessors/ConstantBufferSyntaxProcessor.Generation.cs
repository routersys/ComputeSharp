        ...
        static void ConstantBufferCallback(TInfo info, IndentedTextWriter writer)
        {
            ...
            using (writer.WriteBlock())
            {
                ...
                writer.WriteLineSeparatedMembers(info.Fields.AsSpan(), (field, writer) =>
                {
                    switch (field)
                    {
                        case FieldInfo.Primitive { TypeName: "System.Boolean" or "ComputeWeave.Bool" } primitive:
                        ...
                        case FieldInfo.NonLinearMatrix matrix:
                            string rowTypeName = HlslKnownTypes.GetMappedName($"ComputeWeave.{matrix.ElementName}{matrix.Columns}");
