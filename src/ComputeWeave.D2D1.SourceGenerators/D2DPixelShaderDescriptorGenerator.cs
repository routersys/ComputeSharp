...
public sealed partial class D2DPixelShaderDescriptorGenerator : IIncrementalGenerator
{
    ...
    /// </summary>
    private const string GeneratorName = "ComputeWeave.D2D1.D2DPixelShaderDescriptorGenerator";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValuesProvider<D2D1ShaderInfo> shaderInfoWithNoHlslBytecode =
            ...
            .ForAttributeWithMetadataName(
                "ComputeWeave.D2D1.D2DGeneratedPixelShaderDescriptorAttribute",
                ...
                static (context, token) =>
                {
                    ...
                    INamedTypeSymbol shaderInterfaceType = context.SemanticModel.Compilation.GetTypeByMetadataName("ComputeWeave.D2D1.ID2D1PixelShader")!;
                    ...
                    // Prepare the key to compile the shader afterwards. Compilation is also skipped when the
                    // input has been refused, as the HLSL built for it is not the shader the author asked for.
                    // The compilation itself is deliberately deferred to a dedicated node below, so all shaders
                    // can be compiled in parallel there (the incremental driver invokes transform callbacks
                    // sequentially, so compiling here would serialize all shader compilations).
        ...
        context.RegisterSourceOutput(outputInfo, static (context, item) =>
        {
            ...
            item.Hierarchy.WriteSyntax(
                state: item,
                writer: writer,
                baseTypes: [$"global::ComputeWeave.D2D1.Descriptors.ID2D1PixelShaderDescriptor<{item.Hierarchy.Hierarchy[0].QualifiedName}>"],
            ...
            {
                writer.WriteLine();
                writer.WriteLine("namespace ComputeWeave.D2D1.Generated");
