...
partial class D2DPixelShaderDescriptorGenerator
{
    ...
    private static partial class OutputBuffer
    {
            ...
            out D2D1ChannelDepth channelDepth)
        {
            ...
            if (!structDeclarationSymbol.TryGetAttributeWithFullyQualifiedMetadataName("ComputeWeave.D2D1.D2DOutputBufferAttribute", out AttributeData? attributeData))
            {
            ...
            if (attributeData.ConstructorArguments.Length == 1)
            {
                if (attributeData.AttributeConstructor is IMethodSymbol { Parameters.Length: 1 } constructorSymbol &&
                    constructorSymbol.Parameters[0].Type.HasFullyQualifiedMetadataName("ComputeWeave.D2D1.D2D1BufferPrecision"))
                {
