...
partial class DiagnosticDescriptors
{
    ...
    public static readonly DiagnosticDescriptor InvalidDiscoveredType = new(
        ...
        messageFormat: "The D2D1 shader of type {0} uses the invalid type {1} (only some .NET primitive types, HLSL primitive, vector and matrix types, and custom types containing these types can be used, and custom types containing these types can be used, and bool fields in custom struct types have to be replaced with the ComputeWeave.Bool type for alignment reasons)",
        category: "ComputeWeave.D2D1.Shaders",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true,
    ...
    /// <para>
    /// Format: <c>"The type {0} cannot be indexed in a D2D1 pixel shader (only the HLSL vector and matrix types, the resource types and arrays provide an element access, so the element has to be reached through a field or a method instead)"</c>.
    /// </para>
    /// </summary>
    public static readonly DiagnosticDescriptor InvalidElementAccess = new(
        id: "CMPWD2D0090",
        title: "Invalid element access",
        messageFormat: "The type {0} cannot be indexed in a D2D1 pixel shader (only the HLSL vector and matrix types, the resource types and arrays provide an element access, so the element has to be reached through a field or a method instead)",
        category: "ComputeWeave.D2D1.Shaders",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true,
