...
partial class DiagnosticDescriptors
{
    ...
    public static readonly DiagnosticDescriptor DeclarationWithNoBody = new(
        ...
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true,
        description: "What is written into the generated HLSL is built from the body of the declaration, so one that carries none has nothing to write. An extern declaration is that case, and C# reports only a warning for it. Without this an imported method or local function is written out as a declaration with no body and the shader compiler answers by naming generated code the author never wrote, while an imported constructor and the entry point end the generator instead, which discards the descriptors for every shader in the compilation unit. A declaration split into parts is unaffected, the implementing part being the one that is read.",
        helpLinkUri: "https://github.com/routersys/ComputeWeave");
}
