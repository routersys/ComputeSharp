...
using static ComputeWeave.D2D1.SourceGenerators.Diagnostics.SuppressionDescriptors;
        ...
        foreach (Diagnostic diagnostic in context.ReportedDiagnostics)
        {
            ...
            if (syntaxNode is not null)
            {
                    ...
                    HlslKnownTypes.IsResourceTextureType(fieldSymbol.Type.GetFullyQualifiedMetadataName()))
                {
                    ...
                    if (semanticModel.Compilation.GetTypeByMetadataName("ComputeWeave.D2D1.ID2D1PixelShader") is not { } d2D1PixelShaderSymbol)
                    {
                        continue;
                    }
