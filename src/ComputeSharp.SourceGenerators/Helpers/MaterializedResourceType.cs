...
internal static class MaterializedResourceType
{
    ...
    private static string GetMetadataName(ResourcePlanKind planKind, int typeArgumentCount, ComputeResourceAccess access)
    {
        ...
        {
            return isReadOnly ? "ComputeSharp.ReadOnlyBuffer`1" : "ComputeSharp.ReadWriteBuffer`1";
        }

        if (typeArgumentCount is 1)
        {
            return isReadOnly ? "ComputeSharp.ReadOnlyTexture2D`1" : "ComputeSharp.ReadWriteTexture2D`1";
        }

        return isReadOnly ? "ComputeSharp.ReadOnlyTexture2D`2" : "ComputeSharp.ReadWriteTexture2D`2";
    }
}
