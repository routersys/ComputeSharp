...
partial class PipelineDescriptorGenerator
{
    ...
    private static void WriteHostRegistration(PipelineHostInfo item, IndentedTextWriter writer)
    {
        ...
        writer.WriteLine($"private readonly global::ComputeSharp.ComputeHostRuntime {RuntimeFieldName};");
        writer.WriteLine();
        ...
        writer.WriteLine($"private {item.TypeName}(global::ComputeSharp.GraphicsDevice device, int maximumPendingSubmissions)");

        using (writer.WriteBlock())
        {
            writer.WriteLine($"this.@{item.DeviceFieldName} = device;");
            writer.Write($"this.{RuntimeFieldName} = global::ComputeSharp.ComputeHostRuntime.Create(device, CanonicalDescriptor, maximumPendingSubmissions, [");
        ...
        writer.WriteGeneratedAttributes(GeneratorName);
        writer.WriteLine($"public static {item.TypeName} Create(global::ComputeSharp.GraphicsDevice device, int maximumPendingSubmissions)");
    ...
    private static void WriteHostSlot(OwnedSlotSyntaxInfo slot, IndentedTextWriter writer)
    {
        ...
        if (slot.BindingResourceTypeName is string bindingResourceTypeName)
        {
            ...
            writer.WriteGeneratedAttributes(GeneratorName);
            writer.WriteLine($"{slot.BindingAccessibility} global::ComputeSharp.ComputeResourceBinding<{bindingResourceTypeName}> Get{slot.CanonicalName}ComputeBinding()");
    ...
    private static void WriteSlotMaterializer(OwnedSlotSyntaxInfo slot, IndentedTextWriter writer)
    {
        ...
        writer.WriteGeneratedAttributes(GeneratorName);
        writer.WriteLine($"private readonly struct {slot.MaterializerTypeName} : global::ComputeSharp.IComputeGenerationMaterializer");
        ...
        using (writer.WriteBlock())
        {
            ...
            writer.WriteLine();
            writer.WriteLine("/// <inheritdoc/>");
            writer.WriteLine("public void Materialize(ref global::ComputeSharp.ComputeGenerationContext context)");
