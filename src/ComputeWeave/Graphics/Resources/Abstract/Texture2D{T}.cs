...
using ComputeWeave.Graphics.Resources.Helpers;
using ComputeWeave.Graphics.Resources.Interop;
...
public abstract unsafe partial class Texture2D<T> : IReferenceTrackedObject, IGraphicsResource, ID3D12ReadWriteResource, IResourceGenerationOwner, IGenerationBoundResource
    where T : unmanaged
{
    /// <summary>
    ...
    private protected Texture2D(GraphicsDevice device, int width, int height, ResourceType resourceType, AllocationMode allocationMode, D3D12_FORMAT_SUPPORT1 d3D12FormatSupport)
    {
        ...
        this.memoryAllocation = device.CreateOrAllocateResource(
            ...
            out D3D12_RESOURCE_STATES d3D12ResourceState);

        this.residentState = ComputeGenerationDescriber.GetTrackedState(d3D12ResourceState);
    ...
    private D3D12_RESOURCE_STATES GetD3D12ResourceState()
    {
        ...
        {
            return ComputeGenerationDescriber.GetD3D12ResourceStates(this.generationBinding.ActiveRecord.D3D12State);
        }
    }
