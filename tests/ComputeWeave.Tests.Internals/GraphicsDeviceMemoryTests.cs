...
public unsafe partial class GraphicsDeviceMemoryTests
{
    ...
    public void AccountsEveryCommittedResourceExactly(Device device)
    {
        ...
        using (ReadWriteBuffer<float> buffer = graphicsDevice.AllocateReadWriteBuffer<float>(length))
        {
            ...
            Assert.AreEqual(before.Local.ComputeWeaveOwnedBytes + expectedBytes, during.Local.ComputeWeaveOwnedBytes);
        ...
        Assert.AreEqual(before.Local.ComputeWeaveOwnedBytes, after.Local.ComputeWeaveOwnedBytes);
    }
    ...
    public void AccountsTransferResourcesInTheSegmentTheyAreAllocatedFrom(Device device)
    {
        ...
        using (UploadBuffer<float> buffer = graphicsDevice.AllocateUploadBuffer<float>(1024))
        {
            ...
            {
                Assert.IsTrue(during.Local.ComputeWeaveOwnedBytes > before.Local.ComputeWeaveOwnedBytes);
                Assert.AreEqual(0ul, during.NonLocal.ComputeWeaveOwnedBytes);
            }
            else
            {
                Assert.IsTrue(during.NonLocal.ComputeWeaveOwnedBytes > before.NonLocal.ComputeWeaveOwnedBytes);
                Assert.AreEqual(before.Local.ComputeWeaveOwnedBytes, during.Local.ComputeWeaveOwnedBytes);
            }
        }
        ...
        Assert.AreEqual(before.Local.ComputeWeaveOwnedBytes, after.Local.ComputeWeaveOwnedBytes);
        Assert.AreEqual(before.NonLocal.ComputeWeaveOwnedBytes, after.NonLocal.ComputeWeaveOwnedBytes);
    }
    ...
    public void ReportsInactiveSegmentsWithoutAnyByteValue(Device device)
    {
        ...
        if (graphicsDevice.IsUma)
        {
            ...
            Assert.AreEqual(0ul, statistics.NonLocal.ComputeWeaveOwnedBytes);
    ...
    public void KeepsTheOwnedBytesOfExistingResourcesWhenTheLimitIsLowered(Device device)
    {
        ...
        ulong ownedBytes = graphicsDevice.GetMemoryStatistics().Local.ComputeWeaveOwnedBytes;

        try
        {
            graphicsDevice.SetMemoryPolicy(new GraphicsMemoryPolicy { LocalOwnedHardLimitBytes = 0 });

            Assert.AreEqual(ownedBytes, graphicsDevice.GetMemoryStatistics().Local.ComputeWeaveOwnedBytes);
        }
        finally
        {
            graphicsDevice.SetMemoryPolicy(default);
