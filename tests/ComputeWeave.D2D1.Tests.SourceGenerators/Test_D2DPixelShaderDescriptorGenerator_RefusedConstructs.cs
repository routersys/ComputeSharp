...
public class Test_D2DPixelShaderDescriptorGenerator_RefusedConstructs
{
    ...
    {
        CSharpGeneratorTest<D2DPixelShaderDescriptorGenerator>.VerifyDiagnosticIsReported(ShaderWithMember(member), expectedId);
    }
    ...
    /// <summary>
    /// Builds a pixel shader around a member declaration.
    /// </summary>
    /// <param name="member">The member to declare on the shader type.</param>
    /// <returns>The source of a pixel shader carrying <paramref name="member"/>.</returns>
    private static string ShaderWithMember(string member)
    {
        return $$"""
            using ComputeWeave;
            using ComputeWeave.D2D1;
