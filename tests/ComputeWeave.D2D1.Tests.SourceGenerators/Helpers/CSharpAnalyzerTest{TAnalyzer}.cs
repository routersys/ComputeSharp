    ...
    where TAnalyzer : DiagnosticAnalyzer, new()
{
        ...
        LanguageVersion languageVersion = LanguageVersion.CSharp14)
    {
        ...
        test.TestState.ReferenceAssemblies = TestReferenceAssemblies.Net100;
        test.TestState.AdditionalReferences.Add(MetadataReference.CreateFromFile(typeof(Core::ComputeWeave.Hlsl).Assembly.Location));
#if D2D1_TESTS || D2D1_WINUI_TESTS
#if D2D1_WINUI_TESTS
        test.TestState.AdditionalReferences.Add(MetadataReference.CreateFromFile(typeof(D2D1_WinUI::ComputeWeave.D2D1.WinUI.CanvasEffect).Assembly.Location));
#endif
        test.TestState.AdditionalReferences.Add(MetadataReference.CreateFromFile(typeof(D2D1::ComputeWeave.D2D1.ID2D1PixelShader).Assembly.Location));
#else
        test.TestState.AdditionalReferences.Add(MetadataReference.CreateFromFile(typeof(D3D12::ComputeWeave.IComputeShader).Assembly.Location));
#endif
